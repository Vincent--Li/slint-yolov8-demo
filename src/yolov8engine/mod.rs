...
impl Args {
    fn model(&self) -> anyhow::Result<std::path::PathBuf> {
        let path = match &self.model {
            ...
            None => {
                println!("going to download model from hf.");
...
pub fn run<T: Task>(args: Args) -> anyhow::Result<String> {
    ...
    let mut ret_image_name = String::new();
    for image_name in args.images.iter() {
        ...
        ret_image_name = image_name.to_string_lossy().to_string();
